...
#define SYSCLK 80000000L // Give the Systems the clock frequency (80MHz for PIC32MX440F512H)
...
BYTE fButton = 1;
BYTE running = 1;
BYTE sensorTripped = FALSE;

#define FALSE               0
#define TRUE                1

/**
 * Sets number of lights that are to be turned on.
 * @param x number of lights to be turned on
 */
void setNumLights(int x) {
    numLights = x;
}

/**
 * Starts the countdown to when the police are called from the motion sensor.
 */
void startCountdown() {
    countdown++;
}
...
/**
 * Controls which lights are turned on for password entry.
 */
void passwordLights() {
...
/**
 * Initializes the lights, button, input and output pins.
 */
void initializeIO() {
    ...
    //sets as output
    TRISDbits.TRISD1 = 0; // light r25
    TRISDbits.TRISD2 = 0; // light r26
    TRISDbits.TRISD3 = 0; // light r27
    TRISCbits.TRISC1 = 0; // light r29
    TRISCbits.TRISC2 = 0; // light r31

    //sets as input
    TRISDbits.TRISD6 = 1; //button 1
    TRISDbits.TRISD7 = 1; //button 2
    TRISDbits.TRISD13 = 1; //button 3
    TRISBbits.TRISB15 = 1; //fire button (also center joystick button)

    TRISBbits.TRISB0 = 1; //left
    TRISBbits.TRISB1 = 1; //up
    TRISBbits.TRISB3 = 1; //down
    TRISBbits.TRISB4 = 1; //right
...
int main(void) {
    ...
    while (running) {
        ...
        //take back to homescreen or if already there lockscreen, if on lockscreen nothing happens
        if (PORTBbits.RB15 == 1) {
            fButton = 1;
        }

        //on police screen, disable back button
        if (PORTBbits.RB15 == 0 && getScreenIndex() == 4 && fButton == 1) {
            LATCbits.LATC1 = !LATCbits.LATC1;
            LATCbits.LATC2 = !LATCbits.LATC2;
            LATDbits.LATD3 = !LATDbits.LATD3;
            LATDbits.LATD2 = !LATDbits.LATD2;
            LATDbits.LATD1 = !LATDbits.LATD1;
        } //homescreen takes back to lockscreen
        else if (PORTBbits.RB15 == 0 && getScreenIndex() == 1 && fButton == 1) {
            GDDDemoGoToScreen(0);
            fButton = 0;
            setNumLights(0);
            clearCode();
            sensorTripped = FALSE;
            countdown = 0;
        } //from app to homescreen
        else if (PORTBbits.RB15 == 0 && getScreenIndex() > 1 && fButton == 1 && getScreenIndex() != 4) {
            GDDDemoGoToScreen(1);
            fButton = 0;
            setNumLights(0);
            clearCode();
        ...
        //starts the countdown only if on lockscreen
        if (sensorTripped && getScreenIndex() == 0) {
            startCountdown();
        }

        //calls police only when count is reached, if not user is able to use program like usual
        if (countdown == 1000000 && getScreenIndex() == 0) {
            lightOn();
            countdown = 0;
            GDDDemoGoToScreen(4);
        }

        //if sensor tripped, tell program it is tripped
        if (PORTAbits.RA15 == 0 && sensorTripped == FALSE && getScreenIndex() == 0) {
            sensorTripped = TRUE;
        }
